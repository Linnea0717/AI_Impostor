...
describe('listPools', () => {
  it('returns pool metadata with id and name', () => {
    const pools = listPools()
    expect(pools.some(p => p.id === 'classroom')).toBe(true)
    expect(pools.every(p => !p.id.endsWith('.json'))).toBe(true)
    expect(pools.every(p => typeof p.name === 'string' && p.name.length > 0)).toBe(true)
  })
})

describe('loadPool', () => {
  it('loads a pool by name and returns entries with word and fallback', () => {
    const pool = loadPool('classroom')
